...
; Tray icon
; https://www.iconarchive.com/show/office-icons-by-thalita-torres/clipboard-icon.html
icon         := 'Thalita-Torres-Office-Clipboard.ico'     ; Icon for this program
...
FileExist(icon) && TraySetIcon(icon)                                  ; Set the tray icon for this program
...
paste(str) {                                ; Populate and paste the clipboard
 ...
 Sleep(30), Send('^v')                      ; Paste the clip
 OnClipboardChange clipChanged
 (top) && showGUI(g)                        ; If "on top", then show the GUI again
}
...
notepadSet(str) {                               ; Populate & show the notepad
 ...
 (!WinExist('Notepad' ISGUI)) && notepadShow() ; Show GUI if not already visible
